    ...
    class OverworldScene : Scene
    {
        ...
        private List<Dialog> GetListOfSpeech(float x, float y)
        {
            ...
            if(locationOfNPC == shopNPC)
            {
                return new List<Dialog>
                {
                    new Dialog("Glad to see YOUR face here again. Coming often to the Mall ? Anyhow we here in the OwO-World have two kinds of shops. normal shops where you can buy items for gold. And the Alchemy Steeples where you can transform old useless words into items. but be carefull. those words are unredeemable.", 1, 1),
                    new Dialog("The left one is the normal shop so lets start with it! you'll have the option to buy or sell the four basic types of items( HP, ATK, DEF, and SPD) from there. You use A and D to switch between the Menu Options and press Space to select on. then you can use W and S to put a copy of the item in your shopping car. select as many as you want! it would make the owner happy.. poor guy deserves a brake", 1, 1),
