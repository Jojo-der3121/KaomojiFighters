    ...
    class OverworldScene : Scene
    {
        ...
        public OverworldScene(Vector2 priorPlayerPosition = default) : base()
        {
            ...
            foreach (var element in actualTiledMap.ObjectGroups["Objektebene 1"].Objects)
            {
                ...
                {
                    case "Battle":
                        if (element.Name == "Boss" && SafeFileLoader.GetOwOWorldSafeFile()[0] == 1)
                        {
                            map.AddComponent(new OwOWorldTrigger((int)element.Width, (int)element.Height) { owoWorldTriggerType = OwOWOrldTriggerTypes.Bossbattle }).SetLocalOffset(new Vector2(element.X, element.Y));
                        } 
                        else if (element.Name != "Boss") map.AddComponent(new OwOWorldTrigger((int)element.Width, (int)element.Height) { owoWorldTriggerType = OwOWOrldTriggerTypes.battle }).SetLocalOffset(new Vector2(element.X, element.Y));
                        break;
                    case "Goal":
        ...
        private List<Dialog> GetListOfSpeech(float x, float y)
        {
            ...
            if (locationOfNPC == battleNPC)
            {
                return new List<Dialog>
                {
                    ...
                    new Dialog("In the bottom left you should see a tiny display with a number on it. This is your word deck. like in a Card game. each turn you'll draw 5 new words its your job to form an insult with them in order to defeat your opponent. use W and S to go to a word o your liking and press Space to select it. And allways remember everyone has topics that especially hurt their feelings while other topics barely effect a person, so hit them where it hurts the most.  Basic grammer must be respected tho. And be careful not to confuse verbs with nomen. even though in our language most words can be both. ",2,1),
