...
namespace KaomojiFighters.Scenes.SellerScene
{
    class ShopHUD : RenderableComponent, IUpdatable
    {
        ...
        public void Update()
        {
            if (first.IsPressed && buy || first.IsPressed && !buy && availableItems[0] >= selectedProduct[0] + 1) selectedProduct[0]++;
            if (second.IsPressed && buy || second.IsPressed && !buy && availableItems[1] >= selectedProduct[1] + 1) selectedProduct[1]++;
            if (third.IsPressed && buy || third.IsPressed && !buy && availableItems[2] >= selectedProduct[2] + 1) selectedProduct[2]++;
            if (fourth.IsPressed && buy || fourth.IsPressed && !buy && availableItems[3] >= selectedProduct[3] + 1) selectedProduct[3]++;
